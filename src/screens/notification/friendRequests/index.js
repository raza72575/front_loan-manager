...
const NotificationScreen = ({ navigation }) => {
  ...
  // Inside your NotificationScreen component

// ...

const handleOkAndPayMultiple = async (item) => {
  console.log('Ok button clicked for multiple item:', item);

  try {
    // Assuming `item.multipleNotifyId` is the identifier for the multiple notification
    const response = await fetch(
      `http://${API_SERVER_IP}/BackUpApi/api/PaymentNotifications/seenMultipleNotification?id=${item.Id}`,
      {
        method: 'DELETE',
      }
    );
    ...
    if (response.ok) {
      console.log('Multiple notification seen successfully.');
      // Add any additional logic or UI updates here
    } else {
      throw new Error('Failed to mark multiple notification as seen.');
    }
  } catch (error) {
    console.error('Error marking multiple notification as seen:', error);
    Alert.alert('Error', 'Failed to mark multiple notification as seen. Please try again.');
  }
};

const handleOkAndPaySingle = async (item) => {
  console.log('Ok button clicked for single item:', item);

  try {
    // Assuming `item.singleNotifyId` is the identifier for the single notification
    const response = await fetch(
      `http://${API_SERVER_IP}/BackUpApi/api/PaymentNotifications/seenSingleNotification?id=${item.Id}`,
      {
        method: 'DELETE',
      }
    );
    ...
    if (response.ok) {
      console.log('Single notification seen successfully.');
      // Add any additional logic or UI updates here
    } else {
      throw new Error('Failed to mark single notification as seen.');
    }
  } catch (error) {
    console.error('Error marking single notification as seen:', error);
    Alert.alert('Error', 'Failed to mark single notification as seen. Please try again.');
  }
};
...
// ...
  ...
  const renderMultipleNotificationItem = ({ item }) => {
    return (
      <View style={styles.itemContainer}>
        <Text style={styles.notificationText}>
          {`(Expense:${item.ExpenseName} )Notification from ${item.NotifierName}  please send Amount: ${item.Amount}`}
        </Text>
