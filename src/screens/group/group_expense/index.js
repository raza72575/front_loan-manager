...
import MyCustomDropDown from '../../../components/customDropdown/myCustomDropDown';
// import MultiplyBillPay from '../multiplyBillPay';
...
const GroupExpense = ({navigation}) => {
  ...
  const fetchExpense = async (expName) => {
    try {
      ...
      if (Array.isArray(data) && data.length > 0 && data[0].expenseId !== undefined) {
        // Store expenseId in AsyncStorage
        await AsyncStorage.setItem('expenseId', data[0].expenseId.toString());
      } else {
        console.log('ExpenseId is undefined or not present in the data object.');
      }
    } catch (error) {
      console.error(error);
    }
    ...
    try {
      ...
      if (storedExpenseId) {
        ...
        for (const member of members) {
          ...
          const response = await fetch(
            `http://${API_SERVER_IP}/BackUpApi/api/AlertExpense/createSingleExpenseAlert`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
...
const handleButtonClick = async () => {
  try {
    ...
    const exceededPaidAmount = Object.keys(memberToBePaid).find(
      (key) =>
        parseFloat(memberPaid[key] || 0) > parseFloat(memberToBePaid[key] || 0)
    );

    if (exceededPaidAmount !== undefined && parseFloat(exceededPaidAmount) > 0) {
      // Check if the exceededPaidAmount is defined and greater than 0
      Alert.alert(
        'Invalid Amount',
  ...
  return (
    <View >
      ...
      <View style={{flexDirection:'row', borderWidth:0,width:'40%',justifyContent:'space-between'}}>
      

      <TouchableOpacity 
        onPress={() => navigation.navigate("MultiplyBillPay", { expenseAmount: totalExpenseAmount, myExpenseName: expenseName })}
        >
       
       <Text>Pay Multiple</Text>
          ...
          <View style={styles.paymentContainer}>
            ...
            <View style={styles.detail}>
              <TextInput
                ...
                editable={expenseType !== 'equal'} /* Set editable prop conditionally */
              />
              <TextInput
                style={styles.paymentInput}
                keyboardType="numeric"
              ...
              <TextInput
                style={styles.paymentInput}
                keyboardType="numeric"
                placeholder={`Remaining: ${remainingAmounts[item.UserId] || 0}`}
